...
function createMockExecuteFunctions(overrides: Partial<{
	...
	assertBinaryData: (index: number, prop: string) => { fileName: string; mimeType: string };
	...
	const defaults = {
		...
		httpRequest: async () => ({ id: 'post-id-1', channel_id: 'chan-1', message: 'hello', file_ids: [], create_at: 1234567890000 }),
		assertBinaryData: (_index: number, _prop: string) => ({ fileName: 'test.txt', mimeType: 'text/plain' }),
	...
	return {
		...
		getNodeParameter: merged.getNodeParameter as IExecuteFunctions['getNodeParameter'],
		getCredentials: merged.getCredentials as IExecuteFunctions['getCredentials'],
		getNode: () => ({ name: 'Mattermost', type: 'mattermost', typeVersion: 1, position: [0, 0], id: 'node-1', parameters: {} }),
		continueOnFail: merged.continueOnFail,
		helpers: {
			httpRequest: merged.httpRequest as IExecuteFunctions['helpers']['httpRequest'],
			assertBinaryData: merged.assertBinaryData as IExecuteFunctions['helpers']['assertBinaryData'],
			getBinaryDataBuffer: merged.getBinaryDataBuffer as IExecuteFunctions['helpers']['getBinaryDataBuffer'],
		},
...
describe('Mattermost node description', () => {
	...
	it('files fixedCollection has maxValue of 10', () => {
		const node = new Mattermost();
		const filesProp = node.description.properties.find((p) => p.name === 'files');
	...
	it('posts a plain message and returns normalized output', async () => {
		...
		const ctx = createMockExecuteFunctions({
			...
			httpRequest: async (opts: unknown) => {
				requests.push(opts);
				return { id: 'post-1', channel_id: 'chan-1', message: 'Hello world', file_ids: [], create_at: 111 };
			},
		});
	...
	it('strips trailing slash from baseUrl', async () => {
		...
		const ctx = createMockExecuteFunctions({
			...
			httpRequest: async (opts: unknown) => {
				...
				return { id: 'p', channel_id: 'chan-x', message: '', file_ids: [], create_at: 0 };
			},
		});
	...
	it('includes root_id in post body when set', async () => {
		...
		const ctx = createMockExecuteFunctions({
			...
			httpRequest: async (opts: unknown) => {
				...
				return { id: 'p2', channel_id: 'chan-1', message: 'reply', file_ids: [], create_at: 0 };
			},
		});

		const node = new Mattermost();
		await node.execute.call(ctx);
		expect((bodies[0] as Record<string, unknown>).root_id).toBe('parent-post-id');
	});
});

describe('Mattermost execute — file upload', () => {
	it('uploads files then creates post with file_ids', async () => {
		const requests: Array<{ url: string; method: string; body?: unknown; formData?: unknown }> = [];
		const ctx = createMockExecuteFunctions({
			getNodeParameter: (param: string) => {
				if (param === 'channelId') return 'chan-1';
				if (param === 'message') return 'with file';
				if (param === 'rootId') return '';
				if (param === 'files') return { file: [{ binaryPropertyName: 'data' }] };
				if (param === 'attachments') return {};
				return '';
			},
			httpRequest: async (opts: unknown) => {
				const o = opts as { url: string; method: string; body?: unknown; formData?: unknown };
				requests.push(o);
				if (o.url.includes('/api/v4/files')) {
					return { file_infos: [{ id: 'file-id-1' }] };
				}
				return { id: 'post-1', channel_id: 'chan-1', message: 'with file', file_ids: ['file-id-1'], create_at: 0 };
			},
		});
		...
		expect((requests[1].body as Record<string, unknown>).file_ids).toEqual(['file-id-1']);
...
describe('Mattermost execute — attachments', () => {
	it('builds attachment props correctly', async () => {
		...
		const ctx = createMockExecuteFunctions({
			...
			httpRequest: async (opts: unknown) => {
				bodies.push((opts as { body: unknown }).body);
				return { id: 'p3', channel_id: 'chan-1', message: '', file_ids: [], create_at: 0 };
			},
		});
...
describe('Mattermost execute — error handling', () => {
	...
	it('processes multiple items independently', async () => {
		let callIndex = 0;
		const ctx = createMockExecuteFunctions({
			getInputData: () => [{ json: { n: 1 } }, { json: { n: 2 } }] as INodeExecutionData[],
			getNodeParameter: (param: string) => {
				if (param === 'channelId') return 'chan-1';
			...
			httpRequest: async (opts: unknown) => {
				...
				return { id: `post-${body.message}`, channel_id: 'chan-1', message: body.message, file_ids: [], create_at: 0 };
			},
		});
